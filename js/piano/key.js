class Key {
    ...
    get isLastPosition() {
        return this.position === this.canvas.width / this.width;
    }
    ...
    draw() {
        ...
        this.ctx.fillText(this.keycode, this.x + this.width / 2 - 8, this.y + this.height / 2 + 8);
        this.ctx.closePath();

        if (this.pressed) {
            this.ctx.beginPath();
            const highlight = this.ctx.createLinearGradient(0, this.y - this.height - 20, 0, this.y);
        ...
        (this.isLastPosition) && this.drawLines();
    }
}
